...
addpath(genpath('apps/optimal_excitation/functions_Ambidex/'));
...
sigma     = eye(robot.motors) * 1e-2; % torque covariance
sigma_inv = inv(sigma);

max_fval = inf;

m         = 10;                       % num of trajectory coefs per joints
p_initial = rand(m,robot.motors) - 0.5;  % initial p

% find feasible initial p
while max(max(getConstraint(p_initial,trajectory,robot))) > 0
    p_initial = (rand(m,robot.motors) - 0.5)/2;
end
